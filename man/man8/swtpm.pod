...
that may lead to a dictionary attack lockout upon next startup
(TPM2_Startup()) of the TPM 2. Whether the TPM_PT_LOCKOUT_COUNTER is
actually increased depends on previous commands sent to the TPM 2 as well as
internal state of the TPM 2. Swtpm will make a best-effort and send a
TPM2_Shutdown(SU_STATE) or TPM2_Shutdown(SU_CLEAR) if found necessary.
